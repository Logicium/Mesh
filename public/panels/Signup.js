var Signup = function () {
    ...
    var image = 'public/images/bg1.jpg';
    var url = 'public/images/OrgaPreview.PNG';

    this.backgroundImage = div().addClass('backgroundImage').css(Styles.backgroundImageFull(image));
    this.signupContainer = div();
    this.titleBar = siteBar('Orga','Login',loginClick);
    ...
    this.contentTitle = div().css('padding','15px').text('Orga').css(titleText());
    this.contentText = div().css(Styles.padding({a:'15px',t:'50px'})).height('325px').css(Styles.font({size:'30px',color:'white'}))
        .text("Everything you need to build your organization in one place.");
    this.buttonRow = row();
